...
final class ExtensionXPCService: NSObject, AtollXPCServiceProtocol {
    ...
    nonisolated func requestAuthorization(bundleIdentifier providedBundleIdentifier: String, reply: @escaping (Bool, Error?) -> Void) {
    ...
    nonisolated func checkAuthorization(bundleIdentifier providedBundleIdentifier: String, reply: @escaping (Bool) -> Void) {
        Task { @MainActor [weak self] in
            guard let self else { return }
    ...
    nonisolated func presentLiveActivity(descriptorData: Data, reply: @escaping (Bool, Error?) -> Void) {
    ...
    nonisolated func updateLiveActivity(descriptorData: Data, reply: @escaping (Bool, Error?) -> Void) {
        respond(reply: reply) { service in
            let descriptor = try service.decoder.decode(AtollLiveActivityDescriptor.self, from: descriptorData)
            try ExtensionDescriptorValidator.validate(descriptor)
    ...
    nonisolated func dismissLiveActivity(activityID: String, bundleIdentifier providedBundleIdentifier: String, reply: @escaping (Bool, Error?) -> Void) {
        Task { @MainActor [weak self] in
            guard let self else { return }
            guard self.validate(bundleIdentifier: providedBundleIdentifier, reply: reply) else { return }
    ...
    nonisolated func presentNotchExperience(descriptorData: Data, reply: @escaping (Bool, Error?) -> Void) {
        respond(reply: reply) { service in
    ...
    nonisolated func updateNotchExperience(descriptorData: Data, reply: @escaping (Bool, Error?) -> Void) {
        respond(reply: reply) { service in
            let descriptor = try service.decoder.decode(AtollNotchExperienceDescriptor.self, from: descriptorData)
            try ExtensionDescriptorValidator.validate(descriptor)
    ...
    nonisolated func dismissNotchExperience(experienceID: String, bundleIdentifier providedBundleIdentifier: String, reply: @escaping (Bool, Error?) -> Void) {
        Task { @MainActor [weak self] in
            guard let self else { return }
            guard self.validate(bundleIdentifier: providedBundleIdentifier, reply: reply) else { return }
    ...
    nonisolated private func respond(reply: @escaping (Bool, Error?) -> Void, operation: @escaping @MainActor (ExtensionXPCService) throws -> Void) {
        Task { @MainActor [weak self] in
            guard let self else { return }
            do {
                try operation(self)
