...
final class ExtensionXPCServiceHost: NSObject, NSXPCListenerDelegate {
    ...
    nonisolated func listener(_ listener: NSXPCListener, shouldAcceptNewConnection connection: NSXPCConnection) -> Bool {
        ...
        Task { @MainActor in
            ...
            connection.resume()
            self.clientContexts[ObjectIdentifier(connection)] = ClientContext(connection: connection, bundleIdentifier: bundleIdentifier)
    ...
    nonisolated private func resolveBundleIdentifier(for connection: NSXPCConnection) -> String? {
