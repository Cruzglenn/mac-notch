...
enum SneakContentType: Equatable {
    ...
    case lockScreen
    case capsLock
    case atollNotification(id: UUID)
...
extension SneakContentType {
    static func == (lhs: SneakContentType, rhs: SneakContentType) -> Bool {
        ...
        case (.brightness, .brightness),
             ...
             (.capsLock, .capsLock):
            return true
        case let (.atollNotification(lid), .atollNotification(rid)):
            return lid == rid
...
extension SneakContentType {
    ...
    var isAtollNotification: Bool {
        if case .atollNotification = self {
            return true
        }
        return false
    }
...
class DynamicIslandViewCoordinator: ObservableObject {
    ...
    private static let tabOrder: [NotchViews] = [.home, .shelf, .timer, .stats, .colorPicker, .notes, .clipboard, .terminal, .aiAssistant, .messenger, .extensionExperience]
    ...
    @Default(.enableAIAssistantFeature) private var enableAIAssistantFeature
    ...
    private init() {
        ...
        Defaults.publisher(.enableAIAssistantFeature)
            .receive(on: DispatchQueue.main)
            .sink { [weak self] change in
                self?.handleAIAssistantFeatureToggle(change.newValue)
            }
            .store(in: &cancellables)
        ...
        Publishers.MergeMany(
            ...
            Defaults.publisher(.enableAIAssistantFeature).map { _ in () }.eraseToAnyPublisher(),
    ...
    private func handleAIAssistantFeatureToggle(_ isEnabled: Bool) {
        guard !isEnabled, currentView == .aiAssistant else { return }
        withAnimation(.smooth) {
            currentView = .home
        }
    }
    ...
    func toggleSneakPeek(
        ...
        let bypassedTypes: [SneakContentType] = [.music, .timer, .reminder, .bluetoothAudio, .atollNotification(id: UUID())]
        
        // Check if it's an extension or notification type
        let isSpecialType: Bool
        switch type {
        case .extensionLiveActivity, .atollNotification:
            isSpecialType = true
        default:
            isSpecialType = false
        }
        
        if !isSpecialType && !bypassedTypes.contains(type) && !Defaults[.enableSystemHUD] {
            return
        }
        DispatchQueue.main.async {
            let animation: Animation
            if case .atollNotification = type {
