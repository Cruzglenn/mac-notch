...
//    private var windowSizeUpdateWorkItem: DispatchWorkItem?
    ...
    private func calculateRequiredNotchSize() -> CGSize {
        ...
        if coordinator.currentView == .timer {
            ...
            baseSize.height = max(400, screenHeight * maxFraction)
        } else if coordinator.currentView == .aiAssistant {
            let screenHeight = NSScreen.main?.visibleFrame.height ?? 800
            let maxFraction = Defaults[.aiAssistantMaxHeightFraction]
            baseSize.height = max(400, screenHeight * maxFraction)
        } else if coordinator.currentView == .messenger {
            let screenHeight = NSScreen.main?.visibleFrame.height ?? 800
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // Observe AI Assistant settings changes
        Defaults.publisher(.enableAIAssistantFeature, options: []).sink { [weak self] _ in
            self?.debouncedUpdateWindowSize()
        }.store(in: &cancellables)

        Defaults.publisher(.aiAssistantMaxHeightFraction, options: []).sink { [weak self] _ in
            self?.debouncedUpdateWindowSize()
        }.store(in: &cancellables)
    ...
    private func registerOptionalShortcutHandlers() {
        ...
        KeyboardShortcuts.onKeyDown(for: .toggleAIAssistantTab) { [weak self] in
            guard let self else { return }
            guard Defaults[.enableShortcuts], Defaults[.enableAIAssistantFeature] else { return }
    ...
    private func updateFeatureShortcutAvailability() {
        ...
        updateShortcut(.toggleAIAssistantTab, isEnabled: Defaults[.enableShortcuts] && Defaults[.enableAIAssistantFeature])
    }
