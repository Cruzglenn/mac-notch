...
class BluetoothAudioManager: ObservableObject {
    ...
    private final class BluetoothPollingTimer {
        static let shared = BluetoothPollingTimer()
    ...
    private init() {
        ...
        // Perform background initialization to avoid blocking MainActor
        Task {
    ...
    deinit {
        Task { @MainActor in
            BluetoothAudioManager.shared.cleanup()
        }
    }
    ...
    private func startPollingForChanges() {
        ...
        let queue = DispatchQueue(label: "com.dynamicisland.bluetooth.poll", qos: .utility)
        ...
        // Keep a reference via associated object using a global to retain the timer
        BluetoothPollingTimer.shared.replace(with: timer)
    }
    ...
    private func checkInitialDevices() {
        ...
        // Perform potentially blocking Bluetooth queries off the main thread
        let pairedDevices: [IOBluetoothDevice] = {
            guard IOBluetoothHostController.default()?.powerState == kBluetoothHCIPowerStateON else {
            ...
            return devices.filter { $0.isConnected() && isAudioDevice($0) }
        }()

        let connectedAudioDevices = pairedDevices

        DispatchQueue.main.async { [weak self] in
            guard let self else { return }
            print("🎧 [BluetoothAudioManager] Found \(connectedAudioDevices.count) connected audio devices")
            self.connectedDevices = connectedAudioDevices.compactMap { self.createBluetoothAudioDevice(from: $0) }
            self.isBluetoothAudioConnected = !self.connectedDevices.isEmpty
            self.refreshBatteryLevelsForConnectedDevices()
            if let lastDevice = self.connectedDevices.last {
    ...
    private func checkForNewlyConnectedDevices() {
        DispatchQueue.global(qos: .utility).async { [weak self] in
            ...
            let currentlyConnectedDevices = pairedDevices.filter { $0.isConnected() && self.isAudioDevice($0) }

            DispatchQueue.main.async {
                for device in currentlyConnectedDevices {
                    let address = device.addressString ?? "Unknown"
                    if !self.connectedDevices.contains(where: { $0.address == address }) {
                        print("🎧 [BluetoothAudioManager] 🎉 New audio device connected: \(device.name ?? "Unknown")")
                        guard let audioDevice = self.createBluetoothAudioDevice(from: device) else { continue }
                        self.connectedDevices.append(audioDevice)
                        self.lastConnectedDevice = audioDevice
                        self.isBluetoothAudioConnected = true
                        self.refreshBatteryLevelsForConnectedDevices()
                        if let refreshedDevice = self.connectedDevices.last {
                            self.showDeviceConnectedHUD(refreshedDevice)
                        } else {
                            self.showDeviceConnectedHUD(audioDevice)
                        }
                    }
                }
            }
    ...
    private func updateConnectedDevices() {
        DispatchQueue.global(qos: .utility).async { [weak self] in
            ...
            DispatchQueue.main.async {
                let removedDevices = self.connectedDevices.filter { device in
                ...
                if !removedDevices.isEmpty {
                    ...
                    removedDevices.forEach { self.cancelHUDBatteryWait(for: $0) }
                }
                self.isBluetoothAudioConnected = !self.connectedDevices.isEmpty
                self.refreshBatteryLevelsForConnectedDevices()
            }
    ...
    private func handleDeviceDisconnected(_ notification: Notification) {
        ...
        // Ensure UI-isolated methods are called on MainActor
        for d in removed {
            self.cancelHUDBatteryWait(for: d)
        }
    ...
    nonisolated private func isAudioDevice(_ device: IOBluetoothDevice) -> Bool {
    ...
    private func applyConnectedDeviceBatteryLevels(triggerPmsetFallback: Bool = true) {
        ...
        for device in connectedDevices {
            ...
            if bestBatteryLevel(for: device) != nil {
    ...
    nonisolated private func collectPmsetAccessoryBatteryEntries() -> [PmsetAccessoryBatteryEntry] {
        let process = Process()
    ...
    nonisolated private func normalizeProductName(_ name: String) -> String {
        let components = name
            .lowercased()
