...
private enum SettingsTab: String, CaseIterable, Identifiable {
    ...
    var group: SettingsTabGroup {
        ...
        case .clipboard, .screenAssistant, .colorPicker, .shelf,
             .downloads, .shortcuts, .messenger:                              return .utilities
        case .stats, .terminal, .aiAssistant:                                return .developer
    ...
    var title: String {
        ...
        case .aiAssistant: return String(localized: "AI Assistant")
    ...
    var tint: Color {
        ...
        case .aiAssistant: return .gray
        case .messenger: return .gray
...
struct SettingsView: View {
    ...
    private var settingsSearchIndex: [SettingsSearchEntry] {
        [
            ...
            SettingsSearchEntry(tab: .screenAssistant, title: "Vendor Tax (UK VAT)", keywords: ["vendor tax", "vat", "invoice", "receipt", "dext", "xero", "tax"], highlightID: SettingsTab.screenAssistant.highlightID(for: "Vendor Tax (UK VAT)")),
            ...
            // AI Assistant
            SettingsSearchEntry(tab: .aiAssistant, title: "Enable AI Assistant", keywords: ["ai assistant", "chat", "gpt", "intelligence"], highlightID: SettingsTab.aiAssistant.highlightID(for: "Enable AI Assistant")),
            SettingsSearchEntry(tab: .aiAssistant, title: "Sticky mode", keywords: ["ai assistant", "sticky", "keep open"], highlightID: SettingsTab.aiAssistant.highlightID(for: "Sticky mode")),
            SettingsSearchEntry(tab: .aiAssistant, title: "Maximum height", keywords: ["ai assistant", "height", "size"], highlightID: SettingsTab.aiAssistant.highlightID(for: "Maximum height")),

            // Messenger
            SettingsSearchEntry(tab: .messenger, title: "Enable Messenger", keywords: ["messenger", "facebook", "chat", "reply"], highlightID: SettingsTab.messenger.highlightID(for: "Enable Messenger")),
            SettingsSearchEntry(tab: .messenger, title: "Sticky mode", keywords: ["messenger", "sticky", "keep open"], highlightID: SettingsTab.messenger.highlightID(for: "Sticky mode")),
            SettingsSearchEntry(tab: .messenger, title: "Maximum height", keywords: ["messenger", "height", "size"], highlightID: SettingsTab.messenger.highlightID(for: "Maximum height")),
        ]
    }

    private func isTabVisible(_ tab: SettingsTab) -> Bool {
        switch tab {
        case .timer, .stats, .clipboard, .screenAssistant, .colorPicker, .shelf, .notes, .terminal, .aiAssistant, .messenger:
    ...
    private func detailView(for tab: SettingsTab) -> some View {
        ...
        case .aiAssistant:
            SettingsForm(tab: .aiAssistant) {
                AIAssistantSettingsView()
            }
        case .messenger:
            SettingsForm(tab: .messenger) {
        ...
        Form {
            ...
            if enableShortcuts {
                ...
                Section {
                    HStack {
                        VStack(alignment: .leading) {
                            KeyboardShortcuts.Recorder("Toggle AI Assistant Tab:", name: .toggleAIAssistantTab)
                                .disabled(!enableShortcuts || !Defaults[.enableAIAssistantFeature])
                            if !Defaults[.enableAIAssistantFeature] {
                                Text("AI Assistant feature is disabled")
                                    .font(.caption)
                                    .foregroundStyle(.secondary)
                                    .padding(.top, 2)
                            }
...
struct ScreenAssistantSettings: View {
    ...
    @Default(.globalSystemPrompt) var globalSystemPrompt
    @Default(.enablePerAppSystemPrompts) var enablePerAppSystemPrompts
    @Default(.perAppSystemPrompts) var perAppSystemPrompts
    @State private var apiKeyText = ""
    @State private var showingApiKey = false
    @State private var showingAddAppSheet = false
    @State private var editingBundleID: String? = nil
    @State private var editingPromptText = ""
    ...
    var body: some View {
        Form {
            ...
            if enableScreenAssistant {
                Section {
                    Defaults.Toggle(key: .vendorTaxEnabled) {
                        Text("Vendor Tax (UK VAT)")
                    }
                    .help("Automatically classify invoice VAT for Dext/Xero when an invoice is shared")
                    .settingsHighlight(id: highlightID("Vendor Tax (UK VAT)"))
                } header: {
                    Text("Skills")
                } footer: {
                    Text("When enabled, the assistant will automatically detect and classify invoice VAT for Dext/Xero when an invoice is shared.")
                }
        ...
        .sheet(isPresented: $showingAddAppSheet) {
            AddAppPromptSheet(
                onAdd: { bundleID, prompt in
                    systemPromptManager.setPrompt(for: bundleID, prompt: prompt)
    ...
    private var systemPromptsSection: some View {
        Section {
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text("Applied to all AI sessions unless overridden by a per-app prompt.")
                    .font(.caption)
                    .foregroundColor(.secondary)
            }
            
            Defaults.Toggle(key: .enablePerAppSystemPrompts) {
                Text("Enable Per-App System Prompts")
            }
            .settingsHighlight(id: highlightID("Enable Per-App System Prompts"))
            
            if enablePerAppSystemPrompts {
                // Per-app prompt list
                if !perAppSystemPrompts.isEmpty {
                    ForEach(perAppSystemPrompts.sorted(by: { $0.key < $1.key }), id: \.key) { bundleID, prompt in
                        VStack(alignment: .leading, spacing: 6) {
                            HStack {
                                ...
                                Button("Edit") {
                                    ...
                                    editingPromptText = prompt
                                }
                                .buttonStyle(.bordered)
                                .controlSize(.small)
                                
                                Button(role: .destructive) {
                                    systemPromptManager.removePrompt(for: bundleID)
                                } label: {
                            ...
                            if editingBundleID == bundleID {
                                VStack(alignment: .leading, spacing: 6) {
                                    ...
                                    HStack {
                                        ...
                                        Button("Save") {
                                            systemPromptManager.setPrompt(for: bundleID, prompt: editingPromptText)
                                            editingBundleID = nil
                                            editingPromptText = ""
                                        }
                                        .buttonStyle(.borderedProminent)
            ...
            HStack {
                ...
                HStack(spacing: 4) {
                    Circle()
                        .fill(systemPromptManager.activeSystemPromptLabel == "None" ? Color.gray : Color.green)
                        .frame(width: 8, height: 8)
                    Text(systemPromptManager.activeSystemPromptLabel)
                        .foregroundColor(.secondary)
                        .font(.caption)
                }
            }
            ...
            Text("System prompts give the AI specific instructions or personality. Per-app prompts automatically override the default when the corresponding app is in focus.")
        }
    }
...
// MARK: - Add App Prompt Sheet

struct AddAppPromptSheet: View {
    ...
    private var runningApps: [(bundleID: String, name: String, icon: NSImage?)] {
        SystemPromptManager.shared.runningApplications()
    }
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            // App selection
            VStack(alignment: .leading, spacing: 8) {
                Toggle("Enter bundle ID manually", isOn: $useManualEntry)
                    .font(.caption)
                
                if useManualEntry {
                    TextField("Bundle ID (e.g. com.apple.Xcode)", text: $manualBundleID)
                        .textFieldStyle(.roundedBorder)
                } else {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                Text("Example: \"You are a coding assistant. Help with Swift and SwiftUI development.\"")
                    .font(.caption)
                    .foregroundColor(.secondary)
            }
            ...
            HStack {
                ...
                Button("Add Prompt") {
                    let bundleID = useManualEntry ? manualBundleID : selectedBundleID
                    onAdd(bundleID, promptText)
                }
                .buttonStyle(.borderedProminent)
                .disabled(effectiveBundleID.isEmpty || promptText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)
            }
        }
        .padding(24)
        .frame(width: 450)
    ...
    private var effectiveBundleID: String {
        useManualEntry ? manualBundleID.trimmingCharacters(in: .whitespacesAndNewlines) : selectedBundleID
    }
}
            ...
            if enableTerminalFeature {
                ...
                Section {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        Slider(value: $terminalMaxHeightFraction, in: 0.2...0.8, step: 0.05)
                    }
...
struct AIAssistantSettingsView: View {
    ...
    @Default(.aiAssistantMaxHeightFraction) var aiAssistantMaxHeightFraction
    @Default(.selectedAIProvider) var selectedAIProvider
    @Default(.selectedAIModel) var selectedAIModel

    private func highlightID(_ title: String) -> String {
        SettingsTab.aiAssistant.highlightID(for: title)
    }

    private var formattedMaxHeight: String {
        "\(Int(aiAssistantMaxHeightFraction * 100))% of screen"
    }

    var body: some View {
        Form {
            Section {
                Defaults.Toggle(key: .enableAIAssistantFeature) {
                    Text("Enable AI Assistant")
                }
                .settingsHighlight(id: highlightID("Enable AI Assistant"))

                if enableAIAssistantFeature {
                    Defaults.Toggle(key: .aiAssistantStickyMode) {
                        VStack(alignment: .leading, spacing: 2) {
                            Text("Keep AI Assistant open until clicked outside")
                            Text("Prevents the assistant from closing when the cursor accidentally leaves the notch area.")
                                .font(.caption)
                                .foregroundStyle(.secondary)
                        }
                    }
                    .settingsHighlight(id: highlightID("Sticky mode"))
                }
            } header: {
                Text("General")
            } footer: {
                Text("Adds a dedicated AI Assistant tab to the notch. The conversation session persists across notch open/close cycles.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
            ...
            if enableAIAssistantFeature {
                Section {
                    ...
                    Text("Configure API keys for each provider in the Screen Assistant settings tab.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                } header: {
                    Text("Model")
                ...
                Section {
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        Slider(value: $aiAssistantMaxHeightFraction, in: 0.2...0.8, step: 0.05)
                    }
                    .settingsHighlight(id: highlightID("Maximum height"))
                } header: {
                    Text("Appearance")
...
struct MessengerSettingsView: View {
    ...
    @Default(.messengerBackgroundSync) var messengerBackgroundSync
    @Default(.messengerStickyMode) var messengerStickyMode
    @Default(.messengerMaxHeightFraction) var messengerMaxHeightFraction

    private func highlightID(_ title: String) -> String {
        SettingsTab.messenger.highlightID(for: title)
    }

    var body: some View {
        Form {
            Section {
                Defaults.Toggle(key: .enableMessengerFeature) {
                    Text("Enable Messenger")
                }
                .settingsHighlight(id: highlightID("Enable Messenger"))

                if enableMessengerFeature {
                    Defaults.Toggle(key: .messengerBackgroundSync) {
                        VStack(alignment: .leading, spacing: 2) {
                            Text("Background Notifications")
                                .settingsHighlight(id: highlightID("Background Notifications"))
                            Text("Keeps Messenger running in the background to receive notifications. Disabling this saves battery.")
                                .font(.caption)
                                .foregroundColor(.secondary)
                        }
                    }

                    Defaults.Toggle(key: .messengerStickyMode) {
                        VStack(alignment: .leading, spacing: 2) {
                            Text("Keep Messenger open until clicked outside")
                            Text("Prevents Messenger from closing when the cursor accidentally leaves the notch area.")
                                .font(.caption)
                                .foregroundStyle(.secondary)
                        }
                    }
                ...
                Text("Adds a dedicated Messenger tab to the notch. You can log in and reply directly from the notch.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
                ...
                Section {
                    VStack(alignment: .leading, spacing: 8) {
                        HStack {
                            Text("Maximum height")
                            Spacer()
                            Text("\(Int(messengerMaxHeightFraction * 100))% of screen")
                                .foregroundStyle(.secondary)
                                .monospacedDigit()
                        }
                        Slider(value: $messengerMaxHeightFraction, in: 0.2...0.8, step: 0.05)
                    }
                } header: {
                    Text("Appearance")
                }
