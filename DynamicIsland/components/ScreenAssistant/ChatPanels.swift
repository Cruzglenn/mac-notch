...
class ChatMessagesPanel: NSPanel {
    ...
    private func setupContentView() {
        ...
        hostingView.applyChatPanelCornerMask(radius: 16)
                        ...
                        if screenAssistantManager.chatMessages.isEmpty {
                            ...
                            if screenAssistantManager.isLoading {
                                HStack(spacing: 12) {
                                    ProgressView()
                                        .scaleEffect(0.8)
                                    Text(screenAssistantManager.loadingStatus)
                                        .font(.body)
                                        .foregroundColor(.secondary)
                                    Spacer()
                                }
...
struct ChatInputView: View {
        ...
        VStack(spacing: 0) {
            ...
            HStack {
                HStack(spacing: 6) {
                    ...
                    if systemPromptManager.activeSystemPromptLabel != "None" {
                        HStack(spacing: 3) {
    ...
    private func sendMessage() {
        ...
        case .appleIntelligence:
            // Apple Intelligence runs on-device, no API key needed
            apiKey = "on-device"
        case .chatGPTWeb, .perplexityWeb:
            // Web browser interface, no API key needed
...
// MARK: - Extension for NSView
extension NSView {
    func applyChatPanelCornerMask(radius: CGFloat) {
        wantsLayer = true
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            VStack(alignment: message.isFromUser ? .trailing : .leading, spacing: 8) {
                ...
                HStack(spacing: 6) {
                    if !message.isFromUser, let provider = message.provider {
