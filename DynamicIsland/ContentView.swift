...
struct ContentView: View {
    ...
    @ObservedObject var notificationManager = AtollNotificationManager.shared
    ...
    @Default(.aiAssistantStickyMode) var aiAssistantStickyMode
    ...
    var dynamicNotchSize: CGSize {
        ...
        if coordinator.currentView == .aiAssistant {
            if intelligenceDropManager.hasActiveDrop {
                let state = intelligenceDropManager.currentState
                if state == .dropped || state == .reading {
                    return CGSize(width: max(baseSize.width, 420), height: 75)
                } else {
                    // Expanded document view
                    let screenHeight = NSScreen.main?.visibleFrame.height ?? 800
                    let maxAllowedHeight = screenHeight * 0.8
                    let targetHeight = min(maxAllowedHeight, max(520, intelligenceDropManager.contentHeight))
                    return CGSize(width: max(baseSize.width, 540), height: targetHeight)
                }
            } else {
                // Dynamic height: up to aiAssistantMaxHeightFraction of screen, min 400pt for chat
                let screenHeight = NSScreen.main?.visibleFrame.height ?? 800
                let maxFraction = Defaults[.aiAssistantMaxHeightFraction]
                let aiHeight = max(400, screenHeight * maxFraction)
                return CGSize(width: baseSize.width, height: aiHeight)
            }
        }

        if coordinator.currentView == .messenger {
            // Dynamic height: up to messengerMaxHeightFraction of screen, min 400pt for chat
            let screenHeight = NSScreen.main?.visibleFrame.height ?? 800
            let maxFraction = Defaults[.messengerMaxHeightFraction]
            let messengerHeight = max(400, screenHeight * maxFraction)
            return CGSize(width: baseSize.width, height: messengerHeight)
        }

        if coordinator.currentView == .terminal {
            // Dynamic height: up to terminalMaxHeightFraction of screen, min 400pt for terminal
            let screenHeight = NSScreen.main?.visibleFrame.height ?? 800
            let maxFraction = Defaults[.terminalMaxHeightFraction]
            let terminalHeight = max(400, screenHeight * maxFraction)
    ...
    var body: some View {
        ...
        ZStack(alignment: .top) {
            ...
            let interactionLayout = animatedLayout
                .conditionalModifier(interactionsEnabled) { view in
                    view
                        ...
                        .onTapGesture {
                            let isNotchClosed = vm.notchState == .closed
                            let isHapticsEnabled = Defaults[.enableHaptics]
                            if isNotchClosed && isHapticsEnabled {
                ...
                .conditionalModifier(shouldEnableCloseGesture(interactionsEnabled: interactionsEnabled)) { view in
                    view
            ...
            interactionLayout
                ...
                .onChange(of: aiAssistantStickyMode) { _, _ in
                    syncStickyTerminalOutsideClickMonitor()
                }
                .onChange(of: messengerStickyMode) { _, _ in
                    syncStickyTerminalOutsideClickMonitor()
                }
...
//                    .keyboardShortcut("E", modifiers: .command)
          ...
          VStack(alignment: .leading) {
              VStack(alignment: .leading) {
                  if coordinator.firstLaunch {
                      ...
                      } else if isSneakPeekVisibleOnCurrentScreen && Defaults[.inlineHUD] && (coordinator.sneakPeek.type != .music) && (coordinator.sneakPeek.type != .battery) && (coordinator.sneakPeek.type != .timer) && (coordinator.sneakPeek.type != .reminder) && !coordinator.sneakPeek.type.isExtensionPayload && !coordinator.sneakPeek.type.isAtollNotification && ((coordinator.sneakPeek.type != .volume && coordinator.sneakPeek.type != .brightness && coordinator.sneakPeek.type != .backlight) || vm.notchState == .closed) {
                    ...
                    } else if case .atollNotification = coordinator.sneakPeek.type, coordinator.sneakPeek.show, vm.notchState == .closed, let notification = notificationManager.currentNotification, !vm.hideOnClosed {
                        NotificationLiveActivity(notification: notification)
                            .transition(.blurReplace.animation(.interactiveSpring(dampingFraction: 1.2)))
                    } else if (!coordinator.expandingView.show || coordinator.expandingView.type == .privacy) && vm.notchState == .closed && privacyManager.hasAnyIndicator && (Defaults[.enableCameraDetection] || Defaults[.enableMicrophoneDetection]) && !vm.hideOnClosed {                        PrivacyLiveActivity()
                      ...
                      if isSneakPeekVisibleOnCurrentScreen {
                         if (coordinator.sneakPeek.type != .music) && (coordinator.sneakPeek.type != .battery) && (coordinator.sneakPeek.type != .timer) && (coordinator.sneakPeek.type != .reminder) && (coordinator.sneakPeek.type != .capsLock) && !coordinator.sneakPeek.type.isExtensionPayload && !coordinator.sneakPeek.type.isAtollNotification && !Defaults[.inlineHUD] && ((coordinator.sneakPeek.type != .volume && coordinator.sneakPeek.type != .brightness && coordinator.sneakPeek.type != .backlight) || vm.notchState == .closed) {
            ...
            Color.clear
                ...
                .onChange(of: vm.anyDropZoneTargeting) { _, isTargeted in
                    if isTargeted, vm.notchState == .closed {
                        coordinator.currentView = .aiAssistant
                        IntelligenceDropManager.shared.reset()
                        openNotch()
                    } else if !isTargeted {
                        if vm.dropEvent {
                            vm.dropEvent = false
    ...
    /// Installs the global outside-click monitor when Terminal/AI + sticky mode are active (e.g. keyboard-opened tab).
    ...
    private func syncStickyTerminalOutsideClickMonitor() {
        let isTerminalSticky = coordinator.currentView == .terminal && terminalStickyMode
        let isAISticky = coordinator.currentView == .aiAssistant && aiAssistantStickyMode
        let isMessengerSticky = coordinator.currentView == .messenger && messengerStickyMode
        
        guard vm.notchState == .open && (isTerminalSticky || isAISticky || isMessengerSticky) else {
            removeStickyTerminalClickMonitor()
            return
        }
    ...
    private func shouldPreventAutoClose() -> Bool {
        coordinator.firstLaunch || hasAnyActivePopovers() || vm.isAutoCloseSuppressed || SharingStateManager.shared.preventNotchClose || (terminalStickyMode && coordinator.currentView == .terminal) || (aiAssistantStickyMode && coordinator.currentView == .aiAssistant) || (messengerStickyMode && coordinator.currentView == .messenger)
    }
    ...
    private func handleCloseScrollGesture(translation: CGFloat, phase: NSEvent.Phase) {
        // Prevent accidental closure when scrolling in dedicated views
        guard vm.notchState == .open, 
              !vm.isHoveringCalendar, 
              !vm.isScrollGestureActive,
              coordinator.currentView != .messenger else { return }
    ...
    private func shouldEnableCloseGesture(interactionsEnabled: Bool) -> Bool {
        let closeGestureCondition = Defaults[.closeGestureEnabled] || Defaults[.reverseScrollGestures]
        let gesturesEnabled = Defaults[.enableGestures]
        return closeGestureCondition && gesturesEnabled && interactionsEnabled
    }
    ...
    private func shouldFixSizeForSneakPeek() -> Bool {
        ...
        // Check for extension or notification sneak peek
        if case .extensionLiveActivity = coordinator.sneakPeek.type {
            return vm.notchState == .closed && style == .standard
        }
        
        if case .atollNotification = coordinator.sneakPeek.type {
            return vm.notchState == .closed && style == .standard
        }
...
// MARK: - Notch Notifications Extension
...
@MainActor
class AtollNotificationManager: ObservableObject {
    static let shared = AtollNotificationManager()
    
    @Published private(set) var currentNotification: AtollNotification?
    private var notificationQueue: [AtollNotification] = []
    ...
    func post(
        ...
        guard !isDisplaying, !notificationQueue.isEmpty else { return }        
        isDisplaying = true
        let notification = notificationQueue.removeFirst()
        currentNotification = notification
        
        // Trigger the notch via the coordinator
        DynamicIslandViewCoordinator.shared.toggleSneakPeek(
            status: true,
            type: .atollNotification(id: notification.id),
            duration: notification.duration
        )
        
        // Schedule dismissal cleanup
        DispatchQueue.main.asyncAfter(deadline: .now() + notification.duration + 0.5) { [weak self] in
            withAnimation(.smooth) {
...
struct NotificationLiveActivity: View {
    ...
    var body: some View {
        HStack(spacing: 0) {
            // Left Wing: Icon + Title (e.g., "Messenger")
            HStack(spacing: 6) {
                if let icon = notification.iconSystemName {
                    if icon == "Messenger" {
                        Image("Messenger")
                            .resizable()
                            .aspectRatio(contentMode: .fit)
            ...
            .padding(.leading, 8)
            .frame(width: isExpanded ? leftWingWidth : 0, height: wingHeight, alignment: .leading)
            .clipped()

            // Center Segment (Pure black spacer to match notch)
            Rectangle()
                .fill(Color.black)
                .frame(width: currentClosedNotchWidth)

            // Right Wing: Subtitle (e.g., "New Message")
            HStack(spacing: 4) {
            ...
            .padding(.trailing, 8)
            .frame(width: isExpanded ? rightWingWidth : 0, height: wingHeight, alignment: .trailing)
            .clipped()
        }
        .frame(width: isExpanded ? totalEnvelopeWidth : currentClosedNotchWidth, height: vm.effectiveClosedNotchHeight)
        .fixedSize()
        .onAppear {
    ...
    // MARK: - Layout Helpers
    private var wingHeight: CGFloat { max(vm.effectiveClosedNotchHeight - 10, 20) }
    private var currentClosedNotchWidth: CGFloat { 
        let width = (vm.notchState == .closed && vm.notchSize.width > 0) ? vm.notchSize.width : vm.closedNotchSize.width
        return max(0, width)
    }
    
    private var leftWingWidth: CGFloat {
        let textWidth = (notification.title as NSString).size(withAttributes: [.font: NSFont.systemFont(ofSize: 12, weight: .semibold)]).width
        return textWidth + 36 // icon(13) + spacing(6) + padding(8) + extra
    }
    
    private var rightWingWidth: CGFloat {
        guard let subtitle = notification.subtitle else { return 0 }
        let textWidth = (subtitle as NSString).size(withAttributes: [.font: NSFont.systemFont(ofSize: 12, weight: .semibold)]).width
        return textWidth + 16 // padding(8) + extra
    }
    
    private var totalEnvelopeWidth: CGFloat { currentClosedNotchWidth + leftWingWidth + rightWingWidth }
}

// MARK: - Messenger Manager Extension

class BackgroundableWKWebView: WKWebView {
...
@MainActor
class MessengerManager: NSObject, ObservableObject, WKNavigationDelegate {
    static let shared = MessengerManager()
    
    @Published var webView: BackgroundableWKWebView
    private var offscreenWindow: NSWindow?
    private var titleObservation: NSKeyValueObservation?
    private var lastNotifiedCount: Int = 0
    private var lastNotifiedMaxCount: Int = 0
    private var lastNotifiedTitle: String = ""
    private var lastNotificationDate: Date = Date.distantPast
    private var hasInitialized: Bool = false
    private var zeroCountResetTimer: Timer?
    
    private var isViewingMessenger: Bool {
        DynamicIslandViewCoordinator.shared.currentView == .messenger
    }
    
    private override init() {
        let config = WKWebViewConfiguration()
        self.webView = BackgroundableWKWebView(frame: CGRect(x: 0, y: 0, width: 400, height: 600), configuration: config)
        
        // Prevent WebKit from suspending by keeping it in an offscreen window
        let window = NSWindow(
        ...
        window.alphaValue = 0.0 // Invisible
        ...
        self.webView.onWindowChange = { [weak self] newWindow in
            guard let self = self else { return }
            if newWindow == nil {
                // SwiftUI removed it from the view hierarchy. Put it back to the offscreen window if sync is enabled.
                // We defer this slightly to avoid interfering with SwiftUI's tear down
                DispatchQueue.main.async {
                    if self.webView.window == nil && Defaults[.messengerBackgroundSync] {
                        self.offscreenWindow?.contentView?.addSubview(self.webView)
                    }
                }
            }
        }
        ...
        Task {
            for await isSyncEnabled in Defaults.updates(.messengerBackgroundSync) {
                DispatchQueue.main.async {
                    if isSyncEnabled {
                        ...
                        if self.webView.window == self.offscreenWindow {
        ...
        // Initial attach to keep it alive if enabled
        if Defaults[.messengerBackgroundSync] {
            self.offscreenWindow?.contentView?.addSubview(self.webView)
        }
    ...
    private func setupTitleObservation() {
        titleObservation = webView.observe(\.title, options: [.new]) { [weak self] webView, change in
            guard let self = self, let title = change.newValue as? String else { return }
            self.handleTitleChange(title)
        }
    }
    
    func loadMessenger() {
        if let url = URL(string: "https://www.messenger.com") {
    ...
    private func handleTitleChange(_ title: String) {
        let count = parseUnreadCount(from: title)
        
        // Clean title for comparison (e.g. remove "(1)")
        ...
        if !hasInitialized {
            ...
            lastNotifiedMaxCount = count
            lastNotifiedTitle = cleanTitle
            hasInitialized = true
            return
        }
        
        // If viewing messenger, reset baseline silently
        if isViewingMessenger {
            lastNotifiedCount = count
            lastNotifiedMaxCount = count
            lastNotifiedTitle = cleanTitle
            lastNotificationDate = Date()
            zeroCountResetTimer?.invalidate()
            zeroCountResetTimer = nil
            return
        }

        let now = Date()
        let timeSinceLastNotif = now.timeIntervalSince(lastNotificationDate)

        // Hysteresis Logic:
        // If count > lastNotifiedMaxCount, it's a definitely new message.
        if count > lastNotifiedMaxCount {
            if timeSinceLastNotif > 4.0 {
                AtollNotificationManager.shared.post(
                    title: "Messenger",
                    subtitle: "New Message",
                    iconSystemName: "Messenger",
                    duration: 3.0
                )
            ...
            // Invalidate any reset timer as we are active
            zeroCountResetTimer?.invalidate()
            zeroCountResetTimer = nil
            
        } else if count < lastNotifiedMaxCount {
            // Count dropped (flicker or read). We DON'T lower the ceiling immediately.
            // We wait for it to stay low for 15s to prevent flicker-triggering.
            if zeroCountResetTimer == nil {
                zeroCountResetTimer = Timer.scheduledTimer(withTimeInterval: 15.0, repeats: false) { [weak self] _ in
                    Task { @MainActor in
                        if let current = self?.lastNotifiedCount, current < (self?.lastNotifiedMaxCount ?? 0) {
                            self?.lastNotifiedMaxCount = current
                        }
                        self?.zeroCountResetTimer = nil
                    }
                }
            }
        } else if count == lastNotifiedMaxCount && count > 0 && cleanTitle != lastNotifiedTitle && !cleanTitle.lowercased().contains("messenger") {
            // Same count but DIFFERENT PERSON - Notify!
            if timeSinceLastNotif > 5.0 {
                AtollNotificationManager.shared.post(
                    title: "Messenger",
                    subtitle: "New Message",
                    iconSystemName: "Messenger",
        ...
        // Always track the current raw state
        lastNotifiedCount = count
    }

    private func parseUnreadCount(from title: String) -> Int {
        let pattern = "^\\((\\d+)\\)"
        if let range = title.range(of: pattern, options: .regularExpression) {
            let substring = title[range].trimmingCharacters(in: CharacterSet(charactersIn: "()"))
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        let css = """
        /* Force dark mode and pure black */
        body, html, .x1n2onr6, main, section, div { 
            background-color: black !important; 
            color: white !important; 
            border-color: #111 !important;
        }
        
        /* Global zoom to fit notch better */
        body { zoom: 0.8 !important; }

        /* Hide Sidebar and Chat List */
        div[role="navigation"], div[role="grid"], .x1n2onr6.x1ja2u2z.x78zum5.x2lah0s { 
            display: none !important; 
        }
        
        /* Hide top bar / header */
        header, .x1qjc9v5.x9f619.x78zum5.x1nhvcw1.x1n2onr6.xt7dq6l { 
            display: none !important; 
        }
        
        /* Hide the right sidebar (info panel) */
        div.x1n2onr6.x1ja2u2z.x78zum5.x2lah0s.x1qughib.x6s0dn4.x1q0g3np {
            display: none !important;
        }
        
        /* Make the main chat area fill the space */
        div[role="main"], .x9f619.x1n2onr6.x1ja2u2z { 
            width: 100% !important; 
            margin-left: 0 !important; 
            max-width: 100% !important;
        }
        
        /* Hide the 'Messenger' logo/header in login page */
        .x1qjc9v5.x1pi30zi.x1swvt1m.xyamay9, h1 { display: none !important; }
        
        /* Adjust padding for chat messages to be tighter */
        .x1n2onr6.x1ja2u2z.x78zum5.x2lah0s.x1qughib { padding: 0 !important; }
        
        /* Hide 'Get for Desktop' banners */
        .x193iq5w.x1v8589c { display: none !important; }
        """
        
        let script = "var style = document.createElement('style'); style.innerHTML = '\(css.replacingOccurrences(of: "\n", with: " "))'; document.head.appendChild(style);"
