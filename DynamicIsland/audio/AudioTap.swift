...
class AudioTap: NSObject {
    ...
    nonisolated let bridge = AudioBridge()

    // Accessed from the realtime audio callback; keep simple and avoid heavy synchronization.
    // If you need cross-actor mutation guarantees, wrap with an atomics primitive.
    ...
    func restartCapture() {
        ...
        let workItem = DispatchWorkItem { [weak self] in
            ...
            // Re-route to MainActor for these isolated calls
            Task { @MainActor in
    ...
    deinit {
        // Can't call MainActor-isolated stopCaptureSync from nonisolated deinit directly
        // We use Task to ensure cleanup happens on the right actor
        Task { @MainActor in
            AudioTap.shared.stopCaptureSync()
        }
    }
}
